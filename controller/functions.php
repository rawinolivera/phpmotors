<?php
    ...
    function vehicleDetailPage($vehicle, $assocThumbnail) {
        $dv = "<h1>$vehicle[invMake] $vehicle[invModel]</h1>";
        $dv .= "<ul id='inv-display'>";
        foreach($assocThumbnail as $thumbNail){
            $dv .= "<li><img src='$thumbNail[imgPath]' alt='$vehicle[invMake] $vehicle[invModel] thumbnail'></li> ";
        }
        $dv .= "</ul>";
        $dv .= '<div id="inv-display-detail">';
        $money = number_format($vehicle['invPrice'], 2, ".", ",");
        $dv .= '<div id="vImg">'; 
        $dv .= "<img src='$vehicle[imgPath]' alt='Image of $vehicle[invMake] $vehicle[invModel] on phpmotors.com' width='500' height='300'>";
        $dv .= "<p>Price: $$money</p>";
        $dv .= '</div>';
        $dv .= '<div id="vDetail">';
   ...
   // Build images display for image management view
    ...
    {
        $dv = '<ul id="searchList">';
        foreach ($sResults as $vehicle) {
            $dv .= '<li>';
            $dv .= "<h2><a href='/phpmotors/vehicles/?action=pullVehicleData&vehicleId=$vehicle[invId]'><h2>$vehicle[invYear] $vehicle[invMake] $vehicle[invModel]</h2></a>";
    ...
    function buildResultsString($searchString, $sResults)
    {
        $num = count($sResults);
